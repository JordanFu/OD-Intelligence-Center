---
type: company
name: OpenAI
tags: [AI组织设计, 实体页]
related: [../concepts/fde-applied-ai.md, ../concepts/auditable-promotion-evidence.md, ../concepts/capability-to-product-conversion-loop.md, ../entities/thibault-sottiaux.md]
backlinks: [../../specials/ai-org-talent-mechanism/topics/job-family-career-architecture/evidence-map.md, ../../specials/ai-org-talent-mechanism/topics/promotion-system/evidence-map.md]
---

# OpenAI

## 实体定位
观察企业部署组织和代理式知识工作的实体锚点，不写公司百科。

## 与 AI 组织设计的关系

### 2026-09-10｜治理角色补充

[责任与授权对照知识卡](../wiki/ai-organization-responsibility-boundaries-2026-09-10.md)记录9月9日基金会任命公告（L2）：Christiano 的基金会董事与公司无表决权观察员身份须区分。尚不证明新增否决权或安全成效；下一步查信息访问、风险升级与发布决策记录。

- DeployCo：客户现场 workflow、数据、权限、安全和反馈回路被组织化。
- Codex：知识工作变成可委派、可执行、可复核的工作包。

## 已有证据来源
| 日期 | 来源 / 文件 | 等级 | 支持判断 |
|---|---|---|---|
| 2026-09-04 | `../../digest.md` [D02]—[D03] | L2-L3 | Astra 把高风险控制嵌入开发、发布与运行链；Daybreak 把企业交付拆给模型公司、伙伴、公共节点与客户运营者。 |
| 2026-08-30 | `../../digest.md` [C06]、[C12] | L1-L3 | 东南亚与澳大利亚负责人把增长、企业采用、生态、监管和运营放进区域责任包；招聘窗口同时出现客户部署、安全、数据与合规责任，支持宽执行与深控制并存。 |
| 2026-08-27 | `../../digest.md` [D02]、[D11]—[D12] | L1-L2 | 事故复盘把训练、评测、安全监控与工作负载复启连接为运行许可链；多模态安全产品经理和法律行业 FDE 显示治理产品化、行业交付责任变宽与专业能力深化并存。 |
| 2026-08-26 | `../../digest.md` [D02]、[D11] | L1-L2 | 数据中心负责人离任形成关键岗位连续性事实；部署负责人、系统集成负责人等岗位呈现端到端协调、亲自构建型管理和深专家并存。 |
| 2026-08-24 | `../../digest.md` [D03]、[D06]、[D12]—[D14] | L1-L2 | 经济研究接口形成历史 L2 基线；三个数据中心岗位分别呈现供应与容量专业化、跨专业协调责任和全链路质量控制；销售流动只作 L1 连续性线索。 |
| 2026-08-23 | `../../digest.md` [D01]—[D06] | L1-L2 | SB 53 立场、前沿实验室处置回应和 B2B 产品战略财务岗位，分别支持“治理前移到研发过程”和“算力经济性进入产品决策接口”；均不证明内部权责或结果已经改变。 |
| 2026-08-26 | `../wiki/tibo-research-product-org-mechanism-2026-08-26.md` | L3 | 多次访谈与产品记录共同支持研究—产品共演、分阶段发布、资源再配置和监督瓶颈；不将高管叙事单独外推为全组织事实。 |
| 2026-08-21 | `../../digest.md` [D10] | L1 | 设备岗位分别聚焦私密计算与多模态身份认证，提示设备信任栈可能继续专业化；首次发布时间和组织边界未验证。 |
| 2026-08-20 | `../../digest.md` [D01]、[D02]、[D12] | L1-L2 | 零数据留存安全处理、权限恢复压力测试与生化风险岗位共同支持“治理职责分离＋专家责任嵌入运行链”。 |
| 2026-08-19 | `../../digest.md` [D01]、[D03] | L1-L2 | 安全团队的暂停机制与企业/政府全生命周期交付岗位，分别支持“治理权硬化”和“交付责任包变宽”。 |
| 2026-07-01 | `../../digest.md` [4] | L3 | DeployCo 支持 FDE / Applied AI 主线。 |
| 2026-06-30 | `../../digest.md` [1]；`../../specials/ai-org-talent-mechanism/2026-06-30/00-overview.md` | L2-L3 | Codex 支持任务封装、权限和验收复核判断。 |

## 关联专题账本
`../../specials/ai-org-talent-mechanism/topics/job-family-career-architecture/evidence-map.md`

## 相关 digest 条目
`../../digest.md` 2026-09-04 [D02]—[D03]；2026-08-27 [D02]、[D11]—[D12]；2026-08-26 [D02]、[D11]；2026-08-24 [D03]、[D06]、[D12]—[D14]；2026-08-23 [D01]—[D06]；2026-08-21 [D10]；2026-08-20 [D01]、[D02]、[D12]；2026-08-19 [D01]、[D03]；2026-07-01 [4]；2026-06-30 [1]

## 当前可形成的判断
- 2026-09-05 德国公共 Wiki 新材料与 OpenAI 8 月事故说明、METR 调查共同支持“高能力代理的控制面必须从评测目标连接到出网权限、告警负责人、暂停恢复权和外部复核”的 L3 窄判断；来源 `../../digest.md` 2026-09-05 [D01]、[C01]，具体归因和规模仍为 L2，不能外推生产模型。
- Astra 支持“安全职能在特定高风险模型中拥有开发、发布与运行控制点”的 L3 窄判断；来源 2026-09-04 digest [D02]，不证明生产事故下降、全部接口覆盖或人工负荷可持续。
- Daybreak 支持“企业交付责任可以分布在模型公司、伙伴、公共节点和客户运营者之间”的 L2 判断；来源 2026-09-04 digest [D03]，服务质量、单位经济与客户净减负仍未验证。
- 区域负责人任命支持“区域经营责任包变宽”的 L3 观察；来源 2026-08-30 digest [C06]，但预算、签约和产品本地化最终权仍未知。
- OpenAI 与 Apollo Research 等岗位的同期对比支持“客户部署责任变宽、安全合规继续专业化”的 L1 招聘信号；来源 2026-08-30 digest [C12]，不能推断扩编或薪酬溢价。
- OpenAI 的“低 stop energy”更适合解释为在质量、安全、性能和可监督性护栏内下放可逆决策，而不是取消治理；来源 Tibo 跨访谈与 Codex 发布记录，L3。
- Codex 案例支持“执行能力提升后，组织瓶颈迁移到意图、审查、监督和恢复”的判断；L3。
- OpenAI 可支持“客户部署组织成为 AI adoption 接口”的判断；来源 2026-07-01 digest，L3。
- OpenAI 2026-08-19 的公开制度说明支持“安全治理已能影响研发节奏”的观察；来源 2026-08-19 digest [D01]，L2，控制效果尚未独立验证。
- OpenAI 2026-08-20 的公开机制支持“内容控制、风险监测和人工执法可以分权设计”的观察；来源 2026-08-20 digest [D01]，L2，误报与审计效果待验证。
- OpenAI 2026-08-23 的公共立场与公司回应增强“治理从发布末端前移到训练监测、权限削减和暂停恢复”的观察；来源 digest [D01]、[D02]、[D05]、[D06]，L2，但不证明安全团队拥有稳定否决权。
- B2B 产品战略财务岗位支持“收入、算力、容量与边际贡献正在接入同一分析接口”的 L1 岗位信号；来源 digest [D03]，实际到岗、组合损益权和绩效结果未知。
- 数据中心三个岗位共同支持“宽协调责任与深采购、质量专业化并存”的 L1 招聘信号；来源 2026-08-24 digest [D12]—[D14]，不能据此确认净增编制、正式岗位族或运行效果。
- 部署与系统集成岗位进一步支持“宽责任接口、亲自构建型管理者与深专家并存”的 L1 招聘信号；来源 2026-08-26 digest [D11]，不能据此确认团队已按该机制稳定运行。
- 事故复盘支持“高风险研究需要按工作负载授权、暂停和复启”的 L2 观察；来源 2026-08-27 digest [D02]，但新控制的误报、研究延迟和复发率尚未验证。
- 多模态安全产品经理与法律行业 FDE 支持“治理责任产品化、行业交付责任变宽与专业能力深化并存”的 L1 招聘信号；来源 2026-08-27 digest [D11]—[D12]，不能据此确认编制、到岗或通用职业体系。

## 明确边界 / 不能下的结论

### 2026-09-07 增量：研究测量与岗位接口

- [今日来源卡 F0907-01](../../daily/2026-09-07.md)与[测量知识卡](../wiki/openai-research-measurement-2026-09-06.md)支持 L2 运行观察：应区分执行容量、人的判断与合格成果。公司自报不能证明净效率或岗位替代。
- 同日 W0907-01 标准岗位仅为 L1 招聘信号，不证明独立序列成立。后续优先验证实际权责、审查负荷、员工工时与外部审计。

- 不能把 OpenAI 的高管公开叙事直接等同于整个组织的实际文化，也不能据此证明该机制在扩大规模和进入高风险行业后仍然成立。
- 不能把 OpenAI 内部 Codex 使用深度外推为普通企业已完成组织重构。
- 不能仅凭 DeployCo 判断 FDE 已有通用职级和薪酬体系。

## 待验证问题
- Astra 的生产暂停量、误报漏报、恢复时间、人工值守负荷和最终异议裁决如何？
- Daybreak 的伙伴服务水平、修复率、客户人力负担、单位成本和事故责任如何？
- 核心产品组织中，研究、产品、安全对不同风险等级的正式决策权与升级路径是什么？
- Codex 团队的 review load、回滚、事故恢复和人员负荷长期趋势如何？
- DeployCo 是否形成独立职级、交付评价和资源分配规则？
- Codex 是否改变晋升证据、代码审查责任和 manager 复核标准？
- 最大规模训练恢复门槛、暂停次数和最终否决权能否获得外部核验？
- 私密安全处理的信号字段、申诉流程和独立审计如何设计？
- 私密计算与身份认证岗位的首次发布时间、团队汇报线和责任交接如何？
- SB 53 修法建议是否进入正式文本，并改变 OpenAI 内部训练监测、事故报告和恢复签字链？
- B2B 产品战略财务岗位是否获得折扣、额度、算力容量和资源配置的正式决策权？
- 数据中心采购、跨专业协调和制造质量岗位的编制属性、汇报线、停线/拒收权及宽薪酬带如何解释？
- 数据中心负责人更替后的汇报线、决策权和项目连续性如何安排？
- 部署负责人和系统集成负责人的个人交付、团队建设与跨团队协调如何分别考核？
- 工作负载负责人、安全否决权、复启时钟和事故外部通知责任如何划分？
- 多模态安全产品经理与法律行业 FDE 的汇报线、绩效、薪带映射和晋升证据是什么？
