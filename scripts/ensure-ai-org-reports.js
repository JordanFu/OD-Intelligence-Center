...
function dailyContent(kind) {
  const common = `> 云端兜底生成：当本地设备离线或常规自动化未提交时，由 GitHub Actions 生成最低可用版本；正式重跑必须按多代理研究协议执行：四课题专题代理并行、四渠道证据代理并行、主代理交叉证伪。\n\n## 今日一句话结论\n\nAI 组织改革需要同时观察结论层、背景材料（Context）和线索层：高置信材料进入判断，未验证材料保留为追踪线索，不硬凑结论。\n\n## 今日核心判断\n\n1. **AI 运营模式（operating model） 正在从工具层走向组织层。** IBM、Microsoft、Deloitte、McKinsey 的材料共同指向 智能体、人、数据、流程和治理的一体化重构。\n2. **执行层角色会变宽，核心专家能力会变深。** 常规执行工作 被 智能体吸收后，工作流负责人（workflow owner）、智能体治理（agent governance）、AI 基础设施、数据上下文、安全治理、模型评测 等能力更关键。\n3. **岗位体系问题经常是薪酬与激励承接问题。** EY、Payscale 与市场薪酬信号提示 AI 技能溢价需要 技能溢价（skill premium）、项目奖金（project bonus）、长期激励（LTI）、市场校准调薪（market adjustment） 等机制承接。\n\n## 背景材料（Context）\n\n- 企业宣布 AI 采用 不等于完成 岗位与工作流重设计（job/workflow redesign）。Deloitte 的 84% 未重设岗位是关键反例。\n- 中层减少的新闻需要拆解：哪些是协调任务被系统化，哪些是成本控制，哪些是真正的 管理者角色重写。\n- AI 技能溢价（AI skill premium） 需要区分地区、行业、职能、基本薪酬（base pay）、奖金（bonus） 和 长期激励（LTI）。\n\n## 线索层\n\n- 继续追踪 FDE、AI 工作流负责人、智能体运营（agent ops）、AI 治理（AI governance）、基于技能的薪酬（skills-based pay）等头衔 是否形成稳定 岗位族群（job family）。\n- 继续追踪中国公司是否用技能标签、稀缺系数、项目激励替代新增岗位序列。\n`;
  if (kind === 'flat') return md('专题一：组织扁平化与中层减少', `${common}\n## 落地启发\n\n先拆 管理者工作：信息同步、进度追踪、跨部门协调、质量判断、人才发展、冲突处理。可系统化的部分交给 智能体和平台，不可替代的部分升级为 player-coach、工作流负责人（workflow owner） 和治理 owner。`);
  if (kind === 'talent') return md('专题二：高人才密度与复合型人才机制', `${common}\n## 落地启发\n\n高人才密度不再只看个人绩效，而要看 人机协同能动性（human-agent agency）：问题定义、智能体编排、质量治理、复用资产和跨团队推进能力。`);
  if (kind === 'job') return md('专题三：岗位、族群、序列持续建设', `${common}\n## 落地启发\n\n新建岗位/族群前先判断：是长期战略能力，还是短期技能溢价、项目贡献、关键人才保留或 薪酬带宽（pay band） 不足。`);
  if (kind === 'promotion') return md('专题四：未来组织的晋升机制', `${common}\n## 落地启发\n\n晋升证据包应记录业务结果、工作流重设计（workflow redesign）、智能体治理（agent governance）、复用资产和团队能力提升，不记录 AI 使用次数。`);
  return md('AI时代组织与人才机制四课题总览', `${common}\n## 今日新增证据地图\n\n| 渠道 | 用途 |\n|---|---|\n| 官方/一手 | IBM、Microsoft 等 运营模式（operating model） 与 agent 使用信号 |\n| 权威咨询 | Deloitte、McKinsey、Mercer 对 岗位与工作流重设计（job/workflow redesign） 的判断 |\n| 薪酬信号 | EY、Payscale 对 AI 技能溢价（AI skill premium） 与 薪酬治理（pay governance） 的提示 |\n| 知识库 | 知音楼 HR 资料作为 AI+HR、薪酬福利、组织发展精读池 |\n\n## 待验证清单\n\n1. 今日是否存在新的一手公司公告或 SEC 文件。\n2. 是否有新增 JD/薪酬区间支持 AI 技能溢价（AI skill premium）。\n3. 是否有员工/管理者反馈能还原落地争议。\n4. 是否需要把 Context 升级为高置信结论。`);
}

function writeIndex(dir) {
  const date = path.basename(dir);
  writeIfMissing(path.join(dir, 'index.html'), `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>${date} AI时代组织与人才机制四课题日报</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC",sans-serif;background:#0f1117;color:#eef2fb;line-height:1.75;margin:0}main{max-width:1080px;margin:auto;padding:28px 24px}.hero,.card{background:#171a24;border:1px solid #31374b;border-radius:16px;padding:22px;margin:12px 0}.grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.card a,.btn{display:inline-block;color:white;background:#7c92ff;text-decoration:none;border-radius:8px;padding:8px 12px;font-weight:700}.btn{background:#202434;border:1px solid #31374b}@media(max-width:900px){.grid{grid-template-columns:1fr}}</style></head><body><main><a class="btn" href="../../../index.html">返回 OD 情报中心</a><section class="hero"><h1>${date} AI时代组织与人才机制四课题日报</h1><p>云端兜底版：确保本地设备离线时，日报仍会生成、渲染并提交。后续主代理可继续增强。</p><a class="card a" href="./00-overview.html">阅读总览报告</a></section><section class="grid"><article class="card"><h2>组织扁平化</h2><p>中层减少、player-coach、智能体工作流负责人（workflow owner）。</p><a href="./01-flat-organization.html">阅读专题一</a></article><article class="card"><h2>高人才密度</h2><p>人机协同能动性（human-agent agency）、复合型人才、复用资产。</p><a href="./02-talent-density.html">阅读专题二</a></article><article class="card"><h2>岗位族群序列</h2><p>宽岗位、深专家、技能标签与薪酬承接。</p><a href="./03-job-family-career-architecture.html">阅读专题三</a></article><article class="card"><h2>未来晋升机制</h2><p>项目证据、技能溢价、例外通道与校准。</p><a href="./04-promotion-system.html">阅读专题四</a></article></section></main></body></html>`);
}
