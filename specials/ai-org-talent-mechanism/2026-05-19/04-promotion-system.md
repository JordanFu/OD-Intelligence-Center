# 2026-05-19｜专题四：未来组织的晋升机制（Promotion System）
...
AI 贡献必须进入晋升证据包（evidence packet），但**正式晋升仍需要窗口、校准与预算治理**；高频贡献应通过高频认可/项目激励/技能溢价承接，避免把晋升变成“AI 使用次数竞赛”。

## 2. 今日新增事实：只写可追溯事实（注明来源）

- GitLab Handbook 公开说明其晋升与岗位变动流程（含 promotion cycle / calibration 与 out‑of‑cycle 例外路径）。https://handbook.gitlab.com/handbook/people-group/promotions-transfers/
- IBM 长期运行可验证数字徽章（digital badges），由 Credly 平台发放与验证，可作为技能/能力认证的可审计凭证。https://www.credly.com/organizations/ibm/badges
- 中兴通讯官网公开其职业发展与晋升机制信息（含“每年两次晋升评级/积分晋升”口径；用于中国语境对照）。https://job.zte.com.cn/cn/Entering_ZTE/career_development.html
...
1. **“随时晋升”不会替代窗口制晋升；真正会变化的是证据层（可信度：高）。**
   - 证据基础：GitLab 明确以 promotion cycle / calibration 为治理底盘，同时存在 out‑of‑cycle 例外路径；Accenture 的动作（把 AI 工具采用纳入晋升输入）说明企业会把新能力写入“人才讨论输入”，但并未表明取消窗口/校准/预算流程。https://handbook.gitlab.com/handbook/people-group/promotions-transfers/ ｜ https://www.theguardian.com/accenture/2026/feb/19/accenture-links-staff-promotions-to-use-of-ai-tools
...
## 4. 重点案例事实还原：GitLab（窗口校准 + Out-of-cycle 例外）与 IBM（技能徽章）

> 选择理由：两者都具有“公开制度文本/可审计凭证”的特征，能直接支持我们对“混合晋升（窗口为主+例外）”与“能力认证作为门槛证据”的判断。

### 案例A｜GitLab：以 promotion cycle / calibration 为底盘，同时提供 out‑of‑cycle 路径

#### 背景

- 远程优先、强文档化组织更需要可解释的晋升与岗位变动机制，以降低偏差与跨团队不一致。

#### 时间线（以手册为准）

- GitLab Handbook 页面持续维护，并公开“Promotions and Transfers”相关流程。https://handbook.gitlab.com/handbook/people-group/promotions-transfers/

#### 动作/机制（可抽象）

- **窗口制与校准**：以 promotion cycle / calibration 的形式维持标准与预算治理。
- **例外路径**：存在 out‑of‑cycle 的变更路径，用于吸收非周期性的岗位/责任变化（但需要更强的证据与审批治理）。
- **流程化沟通**：把晋升/岗位变化从“口头承诺”变成流程动作（提名、校准、沟通与系统记录）。

#### 结果（公开信息）

- 手册页面未披露量化效果；可用作“机制模板”，其有效性需由组织自有数据验证（晋升满意度、流失率、预算偏差、校准返工率等）。

#### 争议与阻碍

- out‑of‑cycle 如果缺少严格证据标准，会演化为“走关系的快速通道”；如果过于严格，则会丧失留才与即时激励窗口。

### 案例B｜IBM：数字徽章作为可验证的能力凭证

#### 背景

- AI 时代技能更新快，能力证据需要可迁移、可验证、可审计，才能进入人才盘点与晋升证据层。

#### 机制

- IBM 通过 Credly 平台发放与验证 digital badges，使“学习/认证/技能证明”形成外部可验证凭证。https://www.credly.com/organizations/ibm/badges

#### 可借鉴点

- 把徽章定位为“必要非充分条件”：作为晋升输入的门槛/加分项，但不替代影响力与岗位价值评估；并用分层徽章（入门/熟练/专家）绑定到能力模型。
...
- The Guardian（Accenture 与晋升关联）：https://www.theguardian.com/accenture/2026/feb/19/accenture-links-staff-promotions-to-use-of-ai-tools
- The Register（Accenture memo 细节）：https://www.theregister.com/software/2026/02/20/accenture-tells-staffers-if-you-want-a-promotion-use-ai/4311545
